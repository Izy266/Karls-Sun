...
class GameState:
    def __init__(self):
        ...
        self.num_to_piece = ["P", "p", "N", "n", "B", "b", "R", "r", "Q", "q", "K", "k", '.']
        self.piece_to_num = {"P": 0, "p": 1, "N": 2, "n": 3, "B": 4, "b": 5, "R": 6, "r": 7, "Q": 8, "q": 9, "K": 10, "k": 11}
    ...
    def get_enpas(self):
        if self.prev_moves:
            ...
            if (
                (start >> 3, end >> 3) in [(1, 3), (6, 4)] and
                abs(start - end) == 16 and
                self.board[end] >> 1 == 0
            ):
    ...
    def board_to_fen(self):
        ...
        rtc = ''.join(['K' if self.wrtc_king_side else '', 'Q' if self.wrtc_queen_side else '', 'k' if self.brtc_king_side else '', 'q' if self.brtc_queen_side else '']) or "-"
    ...
    def build_fen(self, fen, new=True):
        ...
        self.check = not self.king_safe(self.wlocs[10 + self.cur_player][0] if self.cur_player == 0 else self.blocs[10 + self.cur_player][0], self.cur_player)
    ...
    def promo_move(self, start):
        return (start >> 3 == 6 and self.board[start] == 1) or (start >> 3 == 1 and self.board[start] == 0)
    ...
    def insufficient(self):
        ...
        if any(len(self.wlocs.get(piece, [])) > 0 for piece in major_pieces) or any(len(self.blocs.get(piece, [])) > 0 for piece in major_pieces):
            return False

        # Check for more than one minor piece
        minor_pieces = [2, 3, 4, 5]
        white_minor_pieces = sum(len(self.wlocs.get(piece, [])) for piece in minor_pieces)
        black_minor_pieces = sum(len(self.blocs.get(piece, [])) for piece in minor_pieces)
        ...
        if len(self.wlocs[4]) and len(self.blocs[5]):
            ...
            if self.is_black_square(white_bishop_square) != self.is_black_square(black_bishop_square):
                return False
    ...
    def check_horizontal_attacks(self, loc, side):
        ...
        for off in [1, -1]:
            for new_loc in range(loc + off, ((loc >> 3) << 3) + 8 if off > 0 else ((loc >> 3) << 3) - 1, off):  # Stay within the same rank
                piece = self.board[new_loc]
                if piece in attacking_pieces or (new_loc == loc + off and piece == 10 + side ^ 1): # or king and one square away
                    return False
                elif piece not in [12, 10 + side]:  # Stop if there's a piece blocking the way
                    break
    ...
    def check_vertical_attacks(self, loc, side):
        ...
        # Check down and up
        for off in [8, -8]:
            for new_loc in range(loc + off, 64 if off > 0 else -1, off):  # Stay within the board
                piece = self.board[new_loc]
                if piece in attacking_pieces or (new_loc == loc + off and piece == 10 + side ^ 1): # or king and one square away
                    return False
                elif piece not in [12, 10 + side]:  # Stop if there's a piece blocking the way
    ...
    def check_diagonal_attacks(self, loc, side):
        ...
        for off in [9, -9, 7, -7]:
            ...
            for new_loc in range(loc + off, -1 if off < 0 else 64, off):
                if abs((new_loc >> 3) - (loc >> 3)) != abs((new_loc & 7) - (loc & 7)): # Stop if not on the same diagonal, i.e border is traversed
                    break
                piece = self.board[new_loc]
                if piece in attacking_pieces or (new_loc == loc + off and (piece == 10 + side ^ 1 or pdir and piece == side ^ 1)): # or (king or (pawn and pawn direction)) and one square away 
                    return False
                elif piece not in [12, 10 + side]:  # Stop if there's a piece blocking the way
                    break
    ...
    def check_knight_attacks(self, loc, side):
        ...
        for off in [10, -10, 6, -6, 17, -17, 15, -15]:
            new_loc = loc + off
            if 0 <= new_loc < 64 and abs((new_loc & 7) - (loc & 7)) <= 2:  # Stay within the board without traversing border
                if self.board[new_loc] == attacking_piece:
                    return False
        return True

    def king_safe(self, loc, side):
        check_functions = [self.check_diagonal_attacks, self.check_knight_attacks, self.check_horizontal_attacks, self.check_vertical_attacks]
    ...
    def test_king_safety(self, move):
        ...
        if not self.check:            
            ...
            if piece_start >> 1 != 1:
                if (vertical and end & 7 == kloc & 7) or (horizontal and end >> 3 == kloc >> 3):
                    return True

                if diag and abs((start >> 3) - (end >> 3)) == abs((start & 7) - (end & 7)) and abs((end >> 3) - (kloc >> 3)) == abs((end & 7) - (kloc & 7)):
                    return True

            self.board[start] = 12
            king_safe = (
                self.check_vertical_attacks(kloc, side) if vertical else
                self.check_horizontal_attacks(kloc, side) if horizontal else
                self.check_diagonal_attacks(kloc, side)
            )
        ...
        for off in self.offsets_map[piece_value]:
            ...
            if piece_value == 0:
                if self.on_board(off, start, end):
                    ...
                    if (off in (8, -8) and piece_end != 12) or (off & 7 and (piece_end == 12 or piece_end & 1 == piece_color) and end != self.get_enpas()):
                        continue
                    ...
                    if self.test_king_safety((start, end)):
                        ...
                        # If pawn on home rank
                            ...
                            if self.board[end + off] == 12 and (safe or self.test_king_safety((start, end + off))):
                                if test:
                                    return True
                ...
                if self.on_board(off, start, end):
                    ...
                    if self.king_safe(end, piece_color):
                        ...
                        if not self.check and piece_end == 12 and off in (1, -1):
                            castle = self.brtc_queen_side if piece_color else self.wrtc_queen_side
                            if off == 1:
                                castle = self.brtc_king_side if piece_color else self.wrtc_king_side
                            if castle and all(self.board[start + i * off] == 12 for i in range(2, 3 if off == 1 else 4)) and self.king_safe(start + 2 * off, piece_color):
            ...
            # If piece is queen, rook, bishop or knight
                ...
                while self.on_board(off, start, end):
                    ...
                    if piece_end != 12 or (not safe and not self.check) or piece_value == 1:
                        break
                    end += off
    ...
    def move(self, move):
        ...
        if piece_start >> 1 == 0:
            ...
            # handle promotion            
            if self.promo:
                self.set_piece(end, self.parse_promo(start, move[1])[1] + self.cur_player)
            # if enpassant
        ...
        # if piece is rook
        rook_loc = start if piece_start >> 1 == 3 else end if piece_end >> 1 == 3 else None
        if rook_loc is not None:
            rook_castling_map = {63: 'wrtc_king_side', 56: 'wrtc_queen_side', 7: 'brtc_king_side', 0: 'brtc_queen_side'}
